...
async function startCheckout(countryCode= 'FR') {
  ...
  try {
    ...
    const componentInstance = new Redirect(checkout, {
      type: 'ideal'
    });
...
window.addEventListener('countryChanged', (event) => {
    console.log('Country changed event received in scheme component:', event.detail);
    ...
    if (newCountryId && newCountryId !== cleanCountryId) {
        console.log('Reloading scheme component with new country:', newCountryId);
